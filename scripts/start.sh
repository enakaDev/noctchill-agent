...
tmux send-keys -t "$SESSION_NAME:1" "watch -n 2 cat status/dashboard.md" Enter

# 3. Window 2: アイドル実行環境（4分割）
...
if [ "$WITH_AGENTS" = true ]; then
    echo "エージェント自動起動中..."
    bash "$PROJECT_ROOT/scripts/launch_agents.sh"
    echo ""
fi
