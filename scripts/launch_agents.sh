...
# Producer (Window 0)
echo "  プロデューサー起動中..."
tmux send-keys -t "$SESSION_NAME:0" "claude --system-prompt \"\$(cat $PROMPT_DIR/producer_system.md)\""
tmux send-keys -t "$SESSION_NAME:0" Enter
sleep 3

# Asakura (Window 2, Pane 0)
echo "  浅倉 透 起動中..."
tmux send-keys -t "$SESSION_NAME:2.0" "claude --system-prompt \"\$(cat $PROMPT_DIR/asakura_system.md)\""
tmux send-keys -t "$SESSION_NAME:2.0" Enter
sleep 2

# Higuchi (Window 2, Pane 1)
echo "  樋口 円香 起動中..."
tmux send-keys -t "$SESSION_NAME:2.1" "claude --system-prompt \"\$(cat $PROMPT_DIR/higuchi_system.md)\""
tmux send-keys -t "$SESSION_NAME:2.1" Enter
sleep 2

# Fukumaru (Window 2, Pane 2)
echo "  福丸 小糸 起動中..."
tmux send-keys -t "$SESSION_NAME:2.2" "claude --system-prompt \"\$(cat $PROMPT_DIR/fukumaru_system.md)\""
tmux send-keys -t "$SESSION_NAME:2.2" Enter
sleep 2

# Ichikawa (Window 2, Pane 3)
echo "  市川 雛菜 起動中..."
tmux send-keys -t "$SESSION_NAME:2.3" "claude --system-prompt \"\$(cat $PROMPT_DIR/ichikawa_system.md)\""
