...
プロデューサー（ユーザー）が指示を出すだけで、AIマネージャーが4人のアイドルにタスクを分配し、**スマホからもリアルタイムで進捗を確認**でき、**出先からでも開発が進められる**システムの実現。
...
│   └── manager_prompt.txt          # マネージャー用 Claude プロンプト
│
├── 📂 web/                         # Webダッシュボード（実装予定）
│   └── （実装ディレクトリ）
│
...
- manager_prompt.txt - Claude Code 用プロンプト
...
### マネージャーエージェント（Claude Code）

```
機能:
1. producer_to_manager.yaml を監視
2. プロデューサーからの指示を読み込む
3. タスクを分析・分解
4. 各アイドル用タスクファイル自動生成
...
```
1️⃣ プロデューサー指示
   └─ queue/producer_to_manager.yaml に指示を書き込み
...
> **「プロデューサーは指示と承認だけ。あとはAIに任せる」**

参考元の記事「Claude Codeで『AI部下10人』を作ったら...」と同じく、このプロジェクトもプロデューサーの負担を最小化し、AIエージェント群に仕事を任せるシステムの実現を目指しています。
